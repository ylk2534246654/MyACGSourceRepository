 ...
 */
function search(key) {
	var url = 'http://www.yunxs.com/plus/search.php?q='+ encodeURI(key) + header;
	const response = httpRequest(url);
	
	const list = jsoupArray(response,'.ul_b_list > li').outerHtml();
	var array= [];
	for (var i=0;i<list.length;i++) {
		...
		array.push({
			...
			//概览
			summary : jsoup(data,'p.state').text().match('(?=作者：)(.+)(?=类型：)')[0],
...
function detail(url) {
	...
	return JSON.stringify({
		...
		//作者
		author: jsoup(response,'div.info > ul > li:nth-child(1)').text(),
		
		//日期
		date : jsoup(response,'#cms_favorites').text(),
		
		//概览
		summary: jsoup(response,'.vote').text(),

		//封面
		cover : ToolUtil.urlJoin(url,jsoup(response,'.pic > a > img').attr('src')),
...
function catalog(response,url) {
	...
	//章节代码
	var chapters = jsoupArray(response,'.list_box > ul > li').outerHtml();
...
function content(url) {
	const response = httpRequest(url + header);
	const src = jsoup(response,'.box_box').outerHtml();
	return src;
}
