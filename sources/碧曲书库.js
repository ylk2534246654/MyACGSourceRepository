function manifest() {
	return JSON.stringify({
		...
		//搜索源自动同步更新链接
		syncList: {
			"Gitee":  "https://gitee.com/ylk2534246654/MyACGSourceRepository/raw/master/sources/碧曲书库.js",
			"极狐":   "https://jihulab.com/ylk2534246654/MyACGSourceRepository/-/raw/master/sources/碧曲书库.js",
			...
			"Gitcode":"https://gitcode.net/Cynric_Yx/MyACGSourceRepository/-/raw/master/sources/碧曲书库.js",
		},
		...
		//@NonNull 详情页的基本网址
		baseUrl: "https://www.biqugewx.org",
	});
}
 ...
 */
function search(key) {
	var url = 'https://www.biqugewx.org/modules/article/search.php?searchkey=' + encodeURI(key) + header;
	const response = httpRequest(url);
	
	const list = jsoupArray(response,'tbody > tr:nth-child(n+2)').outerHtml();
	var array= [];
	for (var i=0;i<list.length;i++) {
	    var data = list[i];
		array.push({
			//标题
			title : jsoup(data,'a[href]').text(),
			
			//概览
			summary : jsoup(data,'a[href]:nth-child(2)').text(),
			
			//封面
			//cover : ToolUtil.urlJoin(url,jsoup(data,'.tutuiImg > a > img').attr('src')),
			
			//网址
			url : ToolUtil.urlJoin(url,jsoup(data,'a[href]').attr('href'))
		});
	}
...
function detail(url) {
	...
	return JSON.stringify({
		...
		//日期
		date : jsoup(response,'meta[property=og:novel:update_time]').attr('content'),
		
		//概览
		summary: jsoup(response,'#list > p').text(),
...
function content(url) {
	...
	return booktext.replace(jsoup(response,'#booktext > center').html(),'');
}
