...
const baseUrl = "https://www.biqukun.info";
/**
 * 网站记录
 * www.biqukun.la
 * www.bswtan.com
 ...
 */
function search(key) {
	var url = ToolUtils.urlJoin(baseUrl,'/modules/article/search.php@post->searchkey='+ encodeURI(key) + header);
...
function detail(url) {
	...
	if(response.code() == 200){
		...
		return JSON.stringify({
			...
			//日期
			update: document.selectFirst('#info > p:nth-child(4)').text(),
