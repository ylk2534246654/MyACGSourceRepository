...
//2022/6/18
function search(key) {
	var url = 'https://www.mandao.tv/search.php@post->searchword=' + encodeURI(key) + header;
	const response = httpRequest(url);
	
	const list = jsoupArray(response,'ul > .mb').outerHtml();
	var array= [];
	for (var i=0;i<list.length;i++) {
		...
		array.push({
			...
			//网址
			url : ToolUtil.urlJoin(url,jsoup(data,'.li-hv').attr('href'))
			});
	}
...
function detail(url) {
	...
	return JSON.stringify({
		...
		//概览
		summary: jsoup(response,'#quanjq > p').text(),

		//封面
		cover : ToolUtil.urlJoin(url,jsoup(response,'.pic > img').attr('src')),
...
function catalog(response,url) {
	//目录代码
	const catalogs = jsoupArray(response,'.urlli').outerHtml();
	...
	for (var i=0;i<catalogs.length;i++) {
		...
		//章节代码
		var chapters = jsoupArray(catalog,'.urlli > div > ul > li').outerHtml();
