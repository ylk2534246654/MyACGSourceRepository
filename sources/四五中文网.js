 ...
 */
function search(key) {
	var url = 'https://www.45zw.org/modules/article/search.php?searchkey='+ ToolUtil.encodeURI(key,'gbk') +'&submit=%CB%D1%CB%F7'+ header;
...
function detail(url) {
	...
	return JSON.stringify({
		//标题
		title : jsoup(response,'#info > h1 > :matchText').text(),
		
		//作者
		author: jsoup(response,'#info > h1 > small > a').text(),
		
		//日期
		date : jsoup(response,'#info > div.update  > :matchText:nth-child(3)').text(),
		
		//概览
		summary: jsoup(response,'#intro').text(),

		//封面
		cover : ToolUtil.urlJoin(url,jsoup(response,'#picbox > div > img').attr('src')),
...
function catalog(response,url) {
	...
	//章节代码
	var chapters = jsoupArray(response,'div > dl > dd > a').outerHtml();
