function manifest() {
	return JSON.stringify({
		...
		syncList: {
			...
			"Coding": "https://ylk2534246654.coding.net/p/myacg/d/MyACGSourceRepository/git/raw/master/sources/选书网.js",
 ...
 */
function search(key) {
	var url = 'https://www.xuanshu.com/search.php@post->searchkey='+ encodeURI(key) + header;
...
function detail(url) {
	...
	return JSON.stringify({
		//标题
		title : jsoup(response,'div.info_des > h1').text(),
		
		//作者
		author: jsoup(response,'div.info_des > dl:nth-child(2)').text(),
		
		//日期
		date : jsoup(response,'div.info_des > dl:nth-child(4)').text(),
		
		//概览
		summary: jsoup(response,'div.info_des > div').text(),

		//封面
		cover : ToolUtil.urlJoin(url,jsoup(response,'div.tupian > a > img').attr('src')),
...
function catalog(response,url) {
	...
	//章节代码
	var chapters = jsoupArray(response,'.pc_list > ul > li').outerHtml();
...
function content(url) {
	const response = httpRequest(url + header);
	const src = jsoup(response,'#content1').outerHtml();
	return src;
}
