function manifest() {
	return JSON.stringify({
		...
		syncList: {
			...
			"Gitcode":"https://gitcode.net/Cynric_Yx/MyACGSourceRepository/-/raw/master/sources/妮可动漫.js",
		},
...
function detail(url) {
	...
	return JSON.stringify({
		//标题
		title : jsoup(response,'div.media-body > h2 > a').text(),
		
		//作者
		author: jsoup(response,'div.media-body > dl > dd:nth-child(4)').text(),
		
		//概览
		summary: jsoup(response,'div.m-intro').text(),

		//封面
		cover : jsoup(response,'div.media-left > a > img').attr('data-original'),
