function manifest() {
	return JSON.stringify({
		...
		syncList: {
			...
			"GitLink": "https://www.gitlink.org.cn/api/ylk2534246654/MyACGSourceRepository/raw/sources%2F路漫漫.js?ref=master",
		...
		findList: {
			category: {
				"year": ["全部","2024","2023","2022","2021","2020","2019","2018","2017","2016","2015","2014","更早"],
				"order": {
					"时间排序": "time",
					"人气排序": "hits",
					"评分排序": "score",
 ...
 */
function search(key) {
	var url = JavaUtils.urlJoin(baseUrl, `/vod/search.html?wd=${encodeURI(key)}`);
	var result = [];
	const response = JavaUtils.httpRequest(url);
	if(response.code() == 200){
		const document = response.body().cssDocument();
		var elements = document.select("#mdym> div");
		for (var i = 0;i < elements.size();i++) {
			...
			result.push({
				...
				//最后章节名称
				lastChapterName: element.selectFirst('.label').text(),
...
function find(year, order) {
	if(year == "全部")year = "";
	if(year == "更早")year = "2013-1980";
	var url = JavaUtils.urlJoin(baseUrl, `/vod/show/by/${order}/id/1/year/${year}.html`);
	var result = [];
	const response = JavaUtils.httpRequest(url);
	if(response.code() == 200){
		const document = response.body().cssDocument();
		const elements = document.select("#mdym > div");
		for (var i = 0;i < elements.size();i++) {
			var element = elements.get(i);
			result.push({
				//名称
				...
				//封面网址
				coverUrl: element.selectFirst('.lazyload').absUrl('data-src'),
...
function detail(url) {
	...
	if(response.code() == 200){
		...
		return JSON.stringify({
			...
			//封面网址
			coverUrl: document.selectFirst('.url_img').absUrl('src'),
...
function tocs(document) {
	//目录标签元素选择器
	const tagElements = document.select('.module-tab-content > div');
	
	//目录元素选择器
	const tocElements = document.select('.scroll-content');
	...
	for (var i = 0;i < tocElements.size();i++) {
		...
		//章节元素选择器
		var chapterElements = tocElements.get(i).select('.scroll-content > a');
		...
		for (var i2 = 0;i2 < chapterElements.size();i2++) {
			...
			newChapters.push({
				...
				//章节网址
				url: chapterElement.selectFirst('a').absUrl('href') + "@header->user-agent:"
			});
		}
		newTocs.push({
			//目录名称
			name: tagElements.get(i).selectFirst('div').text(),
			//章节
